...
dataloader = DataLoader(
    ImageDataset("../CycleGAN_Data/%s" % opt.dataset_name, transforms_=transforms_, mode="train"),
...
# ----------ssh -p 35030 root@region-3.autodl.com
#  Training
# ----------
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        loss_psnr = adversarial_loss(real_B, fake_A)

        loss_GAN = (loss_GAN_AB + loss_GAN_BA) / 2

        # psnr
        Psnr = 20 * torch.log10(255 / torch.sqrt_(loss_psnr))
        ...
        # Print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, adv: %f, pixel: %f, cycle: %f] [PSNR: %f] ETA: %s"
            % (
                epoch,
                opt.n_epochs,
                i,
